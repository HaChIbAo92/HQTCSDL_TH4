...
SELECT (NHANVIEN.HONV + ' ' + NHANVIEN.TENLOT + ' '+ NHANVIEN.TENNV) AS 'Họ tên nhân viên có ít nhất thân nhân'
...
--2. Danh  sách  những  nhân  viên không  có  thân nhân nào.
...
WHERE NHANVIEN.MANV = PHONGBAN.TRPHG AND PHONGBAN.TRPHG 
									IN (SELECT THANNHAN.MA_NVIEN 
										FROM NHANVIEN, THANNHAN 
										WHERE NHANVIEN.MANV = THANNHAN.MA_NVIEN)
...
SELECT (NHANVIEN.HONV + ' ' + NHANVIEN.TENLOT + ' ' + NHANVIEN.TENNV) AS 'Họ tên nhân viên có lương trung bình trên mức lương trung bình'
FROM NHANVIEN
WHERE NHANVIEN.LUONG > (SELECT AVG(NHANVIEN.LUONG)
						FROM NHANVIEN, PHONGBAN
...
WHERE NHANVIEN.PHG = DEAN.PHONG AND NHANVIEN.PHG = DIADIEM_PHG.MAPHG 
	AND DEAN.DDIEM_DA LIKE '%Tp. Quảng Ngãi'  AND DIADIEM_PHG.DIADIEM NOT LIKE '%Tp. Quảng Ngãi'

--11. Tìm họ tên và địa chỉ của các nhân viên làm việc cho một đề án ở một địa điểm nhưng lại không sống tại địa điểm đó.
SELECT DISTINCT (NHANVIEN.HONV + ' ' + NHANVIEN.TENLOT + ' ' + NHANVIEN.TENNV) AS 'Họ tên nhân viên', NHANVIEN.DCHI
FROM NHANVIEN, DEAN, DIADIEM_PHG
WHERE NHANVIEN.PHG = DEAN.PHONG AND NHANVIEN.PHG = DIADIEM_PHG.MAPHG 
		AND DEAN.DDIEM_DA IN (SELECT DEAN.DDIEM_DA
								FROM DEAN) 
...
SELECT PHANCONG.MADA
FROM NHANVIEN, PHANCONG
WHERE NHANVIEN.MANV = PHANCONG.MA_NVIEN AND NHANVIEN.MANV = '123'
